    ...
    return {
      ...
      auth: {
        ...
        async verifyCode(email, token) {
          const data = unwrap(await sb.auth.verifyOtp({ email, token, type: "email" }));
          return data.session;
        },
        async signOut() {
    ...
    return {
      ...
      auth: {
        ...
        async sendCode(email) {
          ...
          return { hint: "Demo mode - the code is 123456" };
        },
        async verifyCode(email, token) {
          if (token !== pendingCode && token !== "123456") {
            throw new Error("That code is not correct. In demo mode the code is 123456.");
          }
