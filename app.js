...
(function () {
  ...
  function initAuthScreen() {
    ...
    function setMode(next) {
      ...
      if (mode === "signup") {
        ...
        sub.textContent = "Set a password — you'll use it to sign in from now on.";
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (!session) {
          ...
          err.textContent = "Account created — check your email to confirm it, then sign in.";
          err.hidden = false;
          setMode("signin");
        } else {
  ...
  function openOrderForm(existing, presetCustomer) {
    ...
    const draft = existing
      ? {
          ...
          entry_allowed: existing.entry_allowed === true || existing.entry_allowed === false ? existing.entry_allowed : null,
    ...
    let newCustomer = { first_name: "", last_name: "", phone: "", email: "", notes: "" };
          ...
          <div class="info-card">
            <div class="row stack-sm">
              <div class="field"><label>First Name *</label>
                <input class="input" data-nc="first_name" value="${esc(newCustomer.first_name)}" placeholder="Pim" /></div>
              <div class="field"><label>Last Name *</label>
                <input class="input" data-nc="last_name" value="${esc(newCustomer.last_name)}" placeholder="Charoen" /></div>
            </div>
            <div class="row stack-sm">
            ...
            <div style="display:flex;gap:8px;margin-top:12px">
              ...
              <button type="button" class="btn btn-primary btn-sm" data-save-new-customer>Save tenant</button>
            </div>
          </div>`;
      }
      ...
      return `
        <div class="search-wrap" style="margin-bottom:9px">
          ...
          <input class="input" data-customer-search value="${esc(customerSearch)}"
                 placeholder="Search saved tenants by name, phone, unit or address…" />
        </div>
        ${matches.length
          ...
          : `<p class="hint" style="padding:6px 2px">No saved tenant matches “${esc(customerSearch)}”.</p>`}
        <button type="button" class="btn btn-sm" style="margin-top:9px" data-new-customer-inline>
          ${icon("plus", 15)} New tenant
        </button>`;
    }
    ...
    function render() {
      setModalBody(entry, `
        <div class="field">
          ...
          <input class="input" data-f="site_unit_number" value="${esc(draft.site_unit_number)}" placeholder="12/04" />
        </div>

        <div class="detail-section">
          <h4>Suite Address *</h4>
          ${draft.customer_id && draft.customer
            ? `<div class="selected-customer">
                 ${avatarHTML({ id: draft.customer.id, full_name: draft.customer.name })}
                 <span class="grow">
                   <strong>${esc(draft.customer.name)}</strong>
                   <span class="lines">${esc([draft.customer.phone, draft.customer.email].filter(Boolean).join(" · ") || "No contact")}</span>
                 </span>
                 <button type="button" class="btn btn-sm" data-open-suite>Change</button>
               </div>`
            : `<button type="button" class="select-btn" data-open-suite>
                 <span class="placeholder">Add or search…</span>
                 ${icon("chevD", 16)}
               </button>`}
        </div>

        <div class="field">
          <label>Category of Repair *</label>
          <button type="button" class="select-btn" data-open-category>
            ${draft.category ? esc(draft.category) : `<span class="placeholder">Add or search…</span>`}
            ${icon("chevD", 16)}
          </button>
        </div>
        ...
        <div class="field">
          ...
          <textarea class="textarea" data-f="description"
            placeholder="What needs doing, and anything the crew should know before they arrive">${esc(draft.description)}</textarea>
        </div>

        <div class="field">
          <label>Entry Allowed *</label>
          <div class="chips">
            <button type="button" class="chip ${draft.entry_allowed === false ? "on" : ""}" data-entry="no">No - Call Tenant</button>
            <button type="button" class="chip ${draft.entry_allowed === true ? "on" : ""}" data-entry="yes">Yes</button>
          </div>
        </div>
    ...
    function bindBody() {
      ...
      body.onclick = async (e) => {
        ...
        const entry_ = e.target.closest("[data-entry]");
        if (entry_) { draft.entry_allowed = entry_.dataset.entry === "yes"; return render(); }

        if (e.target.closest("[data-open-category]")) return openCategoryPicker();
        if (e.target.closest("[data-open-suite]")) return openSuiteAddressPicker();
    ...
    /* ---------------------------------------------------------
     * Category of Repair — searchable radio-list picker
     * ------------------------------------------------------- */
    function openCategoryPicker() {
      let query = "";
      let selected = draft.category;
      let subEntry = null;

      subEntry = openModal({
        kind: "category-picker",
        title: "Category of Repair",
        body: "",
        footer: `
          <button class="btn" data-cancel-cat>Cancel</button>
          <span class="spacer"></span>
          <button class="btn btn-primary" data-save-cat>Save</button>`,
        onMount(root, self) {
          subEntry = self;
          subRender();
          $("[data-cancel-cat]", root).addEventListener("click", () => closeModal());
          $("[data-save-cat]", root).addEventListener("click", () => {
      ...
      function subRender() {
        ...
        const base = CFG.CATEGORIES || [];
        const matches = base.filter((c) => !q || c.toLowerCase().includes(q));
        const exact = base.some((c) => c.toLowerCase() === q);
        setModalBody(subEntry, `
          <div class="search-wrap" style="margin-bottom:9px">
            ${icon("search", 17)}
            <input class="input" data-picker-search value="${esc(query)}" placeholder="Add or search" />
          </div>
          <div class="picker-list">
            ${!exact && q ? `<button type="button" class="picker-item add-new" data-pick-value="${esc(query)}">${icon("plus", 14)} Add “${esc(query)}”</button>` : ""}
            ${matches.map((c) => `
              <button type="button" class="picker-item radio ${selected === c ? "on" : ""}" data-pick-value="${esc(c)}">
                <span class="radio-dot"></span> ${esc(c)}
              </button>`).join("")}
          </div>
        `);
        subBind();
      ...
      function subBind() {
        const body = $(".modal-body", subEntry.el);
        body.onclick = (e) => {
          const item = e.target.closest("[data-pick-value]");
          if (item) { selected = item.dataset.pickValue; return subRender(); }
        };
        const search = $("[data-picker-search]", body);
        if (search) {
          search.addEventListener("input", () => {
            query = search.value;
            const pos = search.selectionStart;
            subRender();
            const again = $("[data-picker-search]", $(".modal-body", subEntry.el));
            again.focus();
    ...
    /* ---------------------------------------------------------
     * Suite Address — search / pick a saved tenant, or add new
     * ------------------------------------------------------- */
    function openSuiteAddressPicker() {
      const startCustomerId = draft.customer_id;
      const startCustomer = draft.customer;
      let subEntry = null;
      ...
      subEntry = openModal({
        ...
        body: "",
        footer: `
          <button class="btn" data-cancel-suite>Cancel</button>
          <span class="spacer"></span>
          <button class="btn btn-primary" data-save-suite>Save</button>`,
        onMount(root, self) {
          subEntry = self;
          customerSearch = "";
          showCustomerForm = false;
          subRender();
          $("[data-cancel-suite]", root).addEventListener("click", () => {
            draft.customer_id = startCustomerId;
            draft.customer = startCustomer;
            closeModal();
            render();
          });
          $("[data-save-suite]", root).addEventListener("click", () => {
            closeModal();
            render();
          });
        },
      ...
      function subRender() {
        setModalBody(subEntry, customerSection());
        subBind();
      }
      ...
      function subBind() {
        ...
        body.onclick = async (e) => {
          ...
          if (e.target.closest("[data-new-customer-inline]")) {
            ...
            newCustomer.first_name = customerSearch;
            return subRender();
          }
          ...
          if (e.target.closest("[data-save-new-customer]")) {
            if (!newCustomer.first_name.trim() || !newCustomer.last_name.trim())
              return toast("First and last name are required", true);
            try {
              const created = await DB.customers.create({
                name: `${newCustomer.first_name.trim()} ${newCustomer.last_name.trim()}`.trim(),
                phone: newCustomer.phone || null,
                email: newCustomer.email || null,
                unit_number: draft.site_unit_number || null,
                building: null,
                address: draft.site_address || null,
              ...
              showCustomerForm = false;
              newCustomer = { first_name: "", last_name: "", phone: "", email: "", notes: "" };
              toast("Tenant saved");
              return subRender();
            } catch (ex) {
        ...
        if (search) {
          search.addEventListener("input", () => {
            ...
            const pos = search.selectionStart;
            subRender();
            const again = $("[data-customer-search]", $(".modal-body", subEntry.el));
            again.focus();
            again.setSelectionRange(pos, pos);
          });
        }
    ...
    async function save() {
      if (!draft.category) return toast("Select a category of repair", true);
      if (!draft.description.trim()) return toast("Add a repair description", true);
      if (draft.entry_allowed !== true && draft.entry_allowed !== false)
        return toast("Select whether entry is allowed", true);
      if (!draft.customer_id && !draft.site_address.trim())
        return toast("Pick a saved tenant or add a new one for the suite address", true);

      const suiteLabel = draft.site_unit_number ? `Unit ${draft.site_unit_number}` : "";
      const autoTitle = [draft.category, suiteLabel].filter(Boolean).join(" – ") || draft.description.trim().slice(0, 60) || "Work order";

      const startISO = draft.date ? composeISO(draft.date, draft.time || "09:00") : null;
      const payload = {
        title: existing ? draft.title : autoTitle,
        description: draft.description.trim() || null,
        category: draft.category || null,
        priority: draft.priority,
        entry_allowed: draft.entry_allowed,
   ...
   * With email+password auth, an admin can't create someone else's
   * account without knowing their password — so this just explains
   * the self-serve sign-up flow instead of sending an invite.
   * ========================================================== */
    ...
    openModal({
      ...
      body: `
        <p style="font-size:14.5px;line-height:1.6">
          There's no password for you to set up on their behalf — ask them to open
          this app's link and tap <strong>“Need an account? Create one”</strong> on the
          sign-in screen, using their work email and a password they choose.
        </p>
        <p style="font-size:14.5px;line-height:1.6;margin-top:10px">
          As soon as they've created their account, they'll show up here in
          <strong>Team</strong> — tap their row's role chip anytime to make them
          Admin or Staff.
        </p>`,
